...
export default function LotteryScreen() {
  ...
  const handleSpin = async () => {
    if (!pending?._id) {
      Alert.alert('No chance', 'You do not have any pending lottery chance right now.');
      return;
    }
    ...
    try {
      ...
      const picked = prizes.find((item) => item.label === `₹${amount}`) || prizes[Math.floor(Math.random() * prizes.length)];
      setWinner(picked.id);
      ...
      <ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
          ...
          <View style={styles.grid}>
            {PRIZES.map((prize) => (
              ...
              >
                ...
                <Text style={[styles.cellText, prize.isCenter && styles.cellCenterText]}>
                  {prize.isCenter ? (loading ? '...' : prize.label) : prize.label}
                </Text>
              </TouchableOpacity>
            ))}
          </View>
        ...
        <View style={styles.terms}>
          ...
          <Text style={styles.termsText}>
            Lottery becomes available after your recharge is approved and the linked plan is activated.{'\n'}
            Each successful plan purchase gives exactly one claim opportunity.{'\n'}
            Claimed rewards are added directly to your wallet and income history.
          </Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  cell: {
    width: '31%', aspectRatio: 1, backgroundColor: '#FFFFFF',
    borderRadius: 10, alignItems: 'center', justifyContent: 'center',
  },
  cellCenter: { backgroundColor: '#F5C518' },
  cellWinner: { backgroundColor: '#D4EDDA', borderWidth: 2, borderColor: colors.primary },
  star: { fontSize: 22, color: '#F5C518' },
  cellText: { fontSize: 13, fontWeight: '800', color: '#1A1A1A', textAlign: 'center', marginTop: 2 },
  cellCenterText: { fontSize: 18, fontWeight: '900', color: '#1A1A1A' },
  summaryCard: { backgroundColor: '#FFFFFF', borderRadius: 14, padding: 16, marginBottom: 16 },
  summaryTitle: { fontSize: 16, fontWeight: '900', color: '#1A1A1A' },
  summaryValue: { marginTop: 8, fontSize: 15, color: colors.primary, fontWeight: '800' },
  summaryHint: { marginTop: 8, fontSize: 12, color: '#526052', lineHeight: 18 },
  terms: {
    backgroundColor: '#FFFFFF', borderRadius: 14, padding: 16,
  },
  termsTitle: { fontSize: 18, fontWeight: '900', color: '#1A1A1A', textAlign: 'center', marginBottom: 10 },
  termsText: { fontSize: 13, color: '#333333', lineHeight: 20 },
  historyCard: { backgroundColor: '#FFFFFF', borderRadius: 14, padding: 16, marginTop: 16 },
  historyTitle: { fontSize: 17, fontWeight: '900', color: '#1A1A1A', marginBottom: 10 },
